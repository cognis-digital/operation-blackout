...
**operation-blackout** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `operation-blackout` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
operation-blackout ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
operation-blackout ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
operation-blackout ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `operation-blackout` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="operation-blackout") for rec in records]   # records = your JSON output
